...
import net.liquidpineapple.pang.screens.HardLevels;

/**
 * Class that represents the object for the hard button.
 *
 * @author Jaap-Jan
 * @date 17-10-2016
 */
public class HardButton extends Button {
  ...
  /**
   * Method that starts a hard game when the button is clicked.
   */
  @Override
  public void onClick() {
    Logger.info("I change the screen now");
    HardLevels levels = new HardLevels();
    Application.getBoard().setLevels(levels.createIterator());
    Application.getBoard().changeScreen(new ControlsScreen());
    Application.getBoard().setDifficulty(Difficulty.HARD);
  }
}
